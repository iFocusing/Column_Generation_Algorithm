...
public:
    ...
    void print(){
        std::cout << "MCF problem: " << ProbName << std::endl;
        std::cout << "nNode: " << nNode << std::endl;
        std::cout << "nArcs: " << nArcs << std::endl;
...
public:
    ...
    void fstar(mcfdata &mcf) {
        std::vector<int> numf(mcf.nNode, 0);
        int a = -1;

        // Initialize the vector point and numf
        point[0] = 0;
        int i;
        for (i = 1; i < mcf.nNode; i++){
            point[i] = point[i-1] + mcf.OutDegr[i-1];
        }
        point[i] = mcf.nArcs;

        // Construct the forward star (fill vectors head, cost, and capa)
        for(int i = 0; i < mcf.nArcs; i++){
            a = point[mcf.ArcTail[i]] + numf[mcf.ArcTail[i]];
            head[a] = mcf.ArcHead[i];
            cost[a] = mcf.ArcCost[i];
            modifiedcost[a] = mcf.ArcCost[i];
            capa[a] = mcf.ArcCapa[i];
            numf[mcf.ArcTail[i]] = numf[mcf.ArcTail[i]] + 1;
        }
    }
    ...
    int get_arc_id(int i, int j) {
        //i is tail, j is head;
        int begin_point = point[i];
        for (int k = 0; k <= point[i+1] - begin_point; k++){
            if (head[begin_point+k] == j){
                return begin_point+k;
            }
        }
        return -1;
    }
